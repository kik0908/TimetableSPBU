...
def get_fields_of_study(lng='ru') -> dict:
    """
    ...
    :return:
    """
    response = requests.get(__url, headers=__headers[lng])
...
def get_programs(direction: str, lng='ru') -> dict:
    """
    ...
    :return:
    """
    response = requests.get(f'{__url}/{direction}', headers=__headers[lng])
    if response.status_code != 200:
        raise Exception('Bad request!')
    soup = BeautifulSoup(response.text, 'lxml')
...
def get_groups(direction: str, group_id: str or int, lng='ru') -> dict:
    """
    Get groups
    ...
    :return:
    """
    response = requests.get(f'{__url}/{direction}/StudyProgram/{group_id}', headers=__headers[lng])
    if response.status_code != 200:
        raise Exception('Bad request!')
    soup = BeautifulSoup(response.text, 'lxml')
...
def get_lessons(direction: str, timesheet_id: str or int, start_date: str = '', lng='ru') -> dict:
    """
    Get lessons
    ...
    :return:
    """
    response = requests.get(f'{__url}/{direction}/StudentGroupEvents/Primary/{timesheet_id}/{start_date}',
                            headers=__headers[lng])
    if response.status_code != 200:
        raise Exception('Bad request!')
    soup = BeautifulSoup(response.text, 'lxml')
